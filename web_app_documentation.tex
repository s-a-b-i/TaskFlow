...
\begin{itemize}
    ...
    \item Handling user authentication and secure token management
...
\subsubsection{Axios}
An HTTP client library. Used to make API calls to the Django backend. We have a custom instance \texttt{api.js} that automatically attaches the authentication token to the \texttt{Authorization} header.
...
We use \textbf{Token-based authentication}, which works like this:

\begin{enumerate}
    \item User sends email + password to \texttt{POST /api/auth/login/}
    \item Django validates credentials using \texttt{authenticate()}
    \item On success, the backend generates a unique \textbf{Token} and returns it in the JSON response
    \item The frontend stores this token in \textbf{Local Storage}
    \item On every subsequent request, the frontend attaches this token to the \texttt{Authorization} header (\texttt{Authorization: Token <key>})
    \item The backend verifies the token and identifies the user
\end{enumerate}

\begin{notebox}
We switched to Token-based auth because \textbf{Incognito mode} and modern browsers block third-party cookies by default. Using headers ensures that authentication is stable and reliable across different domains (Vercel to Render).
\end{notebox}

Since we use Token Authentication, standard CSRF protection for API calls is handled by the token itself (as tokens are not automatically sent by browsers like cookies are, they are immune to CSRF). However, we still maintain CSRF awareness for the overall system security.
...
All endpoints are prefixed with \texttt{/api/}. All endpoints (except login/register) require a valid Authorization token header.
...
\midrule
POST & /api/auth/register/ & Register a new user, returns user + Auth Token \\
POST & /api/auth/login/ & Login, returns user + Auth Token \\
...
\begin{itemize}
    ...
    \item Attaches \texttt{Authorization: Token <key>} header to every request
